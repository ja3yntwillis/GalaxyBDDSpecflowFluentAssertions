...
{
    public class RestClientResultsLogger : ILogResults
    {
        ...
        {
            var uri = string.Format("api/run/{0}", runDto.RunId);
        ...
        public TestDto EndTest(string runId, string testId, string status, int attempts, string testData, ErrorDto errorDto, ActionDto actionDto, DateTime endTime)
        {
            ...
            var uri = string.Format("api/run/{0}/test/{1}/error", _runId, testId);
            if (status == "Failed" || status == "Blocked")
            {
                Call<ErrorDto>(HttpMethod.Post, uri, errorDto, ref placeHolder);
            }

            uri = string.Format("api/run/{0}/test/{1}", _runId, testId);
        ...
        public string StartTest(string runId, string testId, string assembly, string fixture, string method, DateTime startTime, string labels)
        {
            ...
            var uri = string.Format("api/run/{0}/test", _runId);
        ...
        public void SendFileSystemResultsToServer(string runId, string resultsDirectory)
        {
            ...
            var remoteRunDto = StartRun(runDto.BaseUrl, runDto.Application, runDto.Attribute, runDto.SuiteType, runDto.Browser, runDto.MaxThreads, runDto.Labels, null, runDto.StartTime, runDto.DatabaseName, runDto.DatabaseUsername);
        ...
        private void Call<T>(HttpMethod httpVerb, string uri, T dataObject, ref string _refVar)
        {
            try
            {
                ...
                {
                    response = $"{RunnerConfiguration.LogApiURL}/{uri}".PostJsonAsync(dataObject)
                ...
                {
                    response = $"{RunnerConfiguration.LogApiURL}/{uri}".PutJsonAsync(dataObject)
                        .ConfigureAwait(false)
                        .GetAwaiter()
                        .GetResult()
                        .ResponseMessage;
                ...
                {
                    _refVar = $"{RunnerConfiguration.LogApiURL}/{uri}".GetStringAsync()
                        .ConfigureAwait(false)
                        .GetAwaiter()
                        .GetResult();
                }
